<?php
    ...
    if(isset($_POST["registro-boton"])){
        ...
        if(!empty($_POST['nombre']) && !empty($_POST['password']) && !empty($_POST['email'])){
            ...
            //$query = $conexion->prepare("SELECT * FROM usuarios1 WHERE EMAIL =:email");
            $query = mysqli_query($conexion, "SELECT * FROM usuarios1 WHERE EMAIL = '$email'");
            ...
            /* if($query->rowCount() == 0){ */
            if(mysqli_num_rows($query) == 0){
                /* $query = $conexion->prepare("INSERT INTO usuarios1 (nombre, email, contrasenia, fk_rol) VALUES (:nombre, :email, :password_hash, 1)"); */
                $result = mysqli_query($conexion, "INSERT INTO usuarios1 (nombre, email, contrasenia, fk_rol, tema_oscuro) VALUES ('$nombre', '$email', '$password_hash', 3, 1)");
...
<body id="body" class="">
    ...
    <div class="pendientes">
        <h3 class="none">Tareas Faltantes</h3>
        <ul class="none">
            <li>Creacion de facturas en pdf con js</li>
            <li>Animaciones de carga durante requerimientos</li>
            <li>Boton agregar/eliminar de favoritos</li>
            <li>Perfil: 
                <ul>
                    <li><small>Modal cambio contraseña</small></li>
                    <li><small>Pantalla ayuda (manual de usuario)</small></li>
                    <li><small>Modal cambiar y cancelar suscripcion</small></li>
                    <li><small>Modal comprobantes</small></li>
                </ul>
            </li>
            <li>ABM: 
                <ul>
                    <li><small>Doble click modificar/eliminar</small></li>
                    <li><small>Verificacion de inputs</small></li>
                    <li><small>Modal de respuesta del query</small></li>
                </ul>
            </li>
        ...
        <h3>ABM</h3>
        <ul>
            <li>Verificacion de inputs</li>
            <li>Margenes inferiores</li>
            <li>Doble click carga de datos en inputs</li>
            <li>Doble click Mostrar botones modi/baja</li>
            <li>Verificacion de inputs</li>
            <li>Modal de respuesta del query</li>
            <li>Carga dinamica del nav segun nivel de acceso</li>
        </ul>
    </div>
    ...
    <section class="login-seccion" id="seccion-login">
        <article class="registro-contenido">
            <div class="registro-contenido-izq">
                ...
                <form id="form-registro" action="RegistroUsuarioPago.php" class="registro-izq-form" method="POST">
                    <div>
